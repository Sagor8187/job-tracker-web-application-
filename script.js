...
function deleted(cardId, title) {
  ...
  const allCards = mainContainer.querySelectorAll(".card");
  allCards.forEach((mainCard) => {
    const cardTitle = mainCard.querySelector("h2").innerText;
    if (cardTitle === title) {
      const statusSpan = mainCard.querySelector("#status");
      statusSpan.innerText = "Not Applied";
      statusSpan.className =
        "bg-gray-200 text-gray-700 text-xs px-3 py-1 rounded-md";
    }
  });

  // counter update tai function call
  calculatcount();
  renderLists();
}
...
document.addEventListener("click", function (event) {
  ...
  if (
    ...
    const typeJob = card.querySelector("#type")?.innerText || "";
    const location = card.querySelector("#location")?.innerText || "";
    const description = card.querySelector("#description")?.innerText || "";

    const jobData = { title, work, type: typeJob, location, description };
    ...
    allCards.forEach((mainCard) => {
      ...
      if (cardTitle === title) {
        ...
        if (target.classList.contains("interview-btn")) {
          ...
          statusSpan.className =
            "bg-green-100 text-green-800 text-xs px-3 py-1 rounded-md";
        } else {
          statusSpan.innerText = "Rejected";
          statusSpan.className =
            "bg-red-100 text-red-800 text-xs px-3 py-1 rounded-md";
        }
      }
    });
